...
pub async fn handle_request(
    ...
    handle_request_internal(state, file_path).await
}
...
fn build_listing_html(
    ...
    html.push_str(&format!("<title>soop3 | {}</title>", escape_html(request_path)));
...
fn resolve_safe_path(
    ...
    join_path_jailed(public_dir, clean_path)
