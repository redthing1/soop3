...
pub async fn authenticate_if_required(
    ...
    let auth_available = state.config.security.username.is_some() 
    ...
    if validate_credentials(&state.config.security, &credentials) {
        debug!("authentication successful for user: {}", credentials.username);
...
pub fn parse_basic_auth(auth_header: &str) -> Result<BasicCredentials, &'static str> {
    ...
    let decoded_str = String::from_utf8(decoded)
        .map_err(|_| "invalid utf8 in credentials")?;
...
pub fn validate_credentials(security_config: &SecurityConfig, credentials: &BasicCredentials) -> bool {
    ...
    constant_time_eq(credentials.username.as_bytes(), expected_username.as_bytes())
        && constant_time_eq(credentials.password.as_bytes(), expected_password.as_bytes())
}
