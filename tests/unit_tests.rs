...
use soop3::{
    config::{AppConfig, SecurityPolicy, SecurityConfig, ServerConfig, load_config_from_file},
    utils::{
        files::{format_file_size, format_timestamp, escape_html, get_mime_type},
...
mod utils_tests {
    ...
    mod path_security_tests {
        ...
        fn test_url_encoded_paths() {
            ...
            assert!(result.exists());
            assert_eq!(result.file_name(), Some(std::ffi::OsStr::new("file with spaces.txt")));
        }
...
mod config_tests {
    ...
    fn test_security_policy_parsing() {
        // test string parsing
        assert_eq!("authenticate_none".parse::<SecurityPolicy>().unwrap(), SecurityPolicy::AuthenticateNone);
        assert_eq!("authenticate_upload".parse::<SecurityPolicy>().unwrap(), SecurityPolicy::AuthenticateUpload);
        assert_eq!("authenticate_all".parse::<SecurityPolicy>().unwrap(), SecurityPolicy::AuthenticateAll);
